...
export class InputManager {
  ...
  _bind() {
    ...
    this.canvas.addEventListener('mousedown', safe(e => this._onStart(this._getPos(e)), 'mousedown'));
    this.canvas.addEventListener('mousemove', safe(e => {
      const pos = this._getPos(e);
      this.hoverCell = pos;
      this._onMove(pos);
    }, 'mousemove'));
    window.addEventListener('mouseup', safe(() => this._onEnd(), 'mouseup'));
    this.canvas.addEventListener('mouseleave', safe(() => {
      this.hoverCell = null;
      if (this.drawing && this.mode === DRAW_MODE.FREEHAND) this.cancelDrawing();
    }, 'mouseleave'));

    this.canvas.addEventListener('touchstart', safe(e => {
      e.preventDefault();
    ...
    this.canvas.addEventListener('touchmove', safe(e => {
      e.preventDefault();
      if (!e.touches || e.touches.length === 0) return;
      const pos = this._getPos(e.touches[0]);
      this.hoverCell = pos;
      if (pos.gx < 0 || pos.gx >= this.grid.width ||
        pos.gy < 0 || pos.gy >= this.grid.height) {
        if (this.mode === DRAW_MODE.FREEHAND) this.cancelDrawing();
        return;
      }
      this._onMove(pos);
    }, 'touchmove'), {passive: false});
    window.addEventListener('touchend', safe(() => this._onEnd(), 'touchend'));
    window.addEventListener('touchcancel', safe(() => this.cancelDrawing(), 'touchcancel'));
  ...
  _isDrawZone(gy) {
    return gy >= this.grid.drawZoneMinY();
  }
  ...
  _onEnd() {
    ...
    if (this.mode === DRAW_MODE.LINE && this.startCell && this.currentCell) {
      ...
      this._line(this.startCell.gx, this.startCell.gy,
        this.currentCell.gx, this.currentCell.gy);
      ...
      if (this._isDrawZone(gy) && this.grid.inBounds(gx, gy)) {
        ...
        if (existing === CELL_TYPE.DEFENSE) {
          ...
          this.defenses.refill(1);
          const wx = this.grid.wrapX(gx);
          this.placedThisDrag = this.placedThisDrag.filter(
            p => !(p.x === wx && p.y === gy)
          );
        }
      }
  ...
  getPreviewCells() {
    ...
    } else if (this.mode === DRAW_MODE.LINE && this.drawing &&
