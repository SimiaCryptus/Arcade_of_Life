...
export class DrawToolsPanel {
    constructor(input) {
        ...
        const diagIds = [
            'pattern-editor-toggle', 'pattern-editor-panel', 'pattern-editor-overlay',
            'exit-to-menu-button', 'pattern-editor', 'pattern-presets',
        ];
    ...
    _syncPresetCombobox() {
        ...
        if (this._activePresetName && !this._editorDirty) {
            ...
            const optionExists = Array.from(sel.options).some(
                o => o.value === this._activePresetName);
    ...
    _openEditorPanel() {
        ...
        if (this.onEditorOpen) {
            try { this.onEditorOpen(); } catch (e) { Logger.error('onEditorOpen handler failed', e); }
        }
    ...
    _closeEditorPanel() {
        ...
        if (this.onEditorClose) {
            try { this.onEditorClose(); } catch (e) { Logger.error('onEditorClose handler failed', e); }
        }
